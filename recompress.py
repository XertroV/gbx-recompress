...
def gen_decompressed(gbx_file: str, suffix: str):
    print(exe_app_banner)
    if not gbx_file.lower().endswith('.gbx'):
        print('!! Error: the input file must end in .gbx (not case sensitive)')
        # raise Exception('file name does not end with .gbx')
        sys.exit(1)
    ...
    logging.info(d_len, d_len_le, c_len, c_len_le)

    logging.info(compressed[:64].hex())
    logging.info(compressed[-16:].hex())
    ...
    with open(out_file, 'bw') as f:
        ...
        f.write(lengths)
        f.write(compressed)
        out_len = len(header) + len(lengths) + len(compressed)

    print(f"""
# Recompressing:

Input: {gbx_file}
Output: {out_file} (in the local directory)

## Recompression status:

- Original file: {len(orig_gbx) // 1000:7d} KB
- Recompressed:  {out_len // 1000:7d} KB
